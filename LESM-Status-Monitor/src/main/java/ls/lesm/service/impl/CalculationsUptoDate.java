		...
		if (salaryRecords == 1)// number_of_salary_records_of_single_employee==1
		{
			...
			if (Date1.isBefore(toDate) && Date2.isAfter(fromDate)) {
				...
				for (Salary s : salary) {
					...
					expenses.setGPM_USD(Math.ceil((Total_salary_from_client[0] - (paid_till_now / 74))));
			...
			if (Date1.isBefore(toDate) && Date2.isAfter(fromDate)) {
				...
				try {
					...
					expenses.setGPM_USD((Total_salary_from_client[0] - Math.ceil(Total_internal_pay / 74)));
